...
* `bin/install_basics` (optional, customizable): Applies basic settings for setting up a machine.
* `bin/install_defaults` (optional, customizable): Applies {mac_os_defaults_link}.
* `bin/install_dev_tools` (required): Installs macOS development tools required by Homebrew.
* `bin/install_extensions` (optional, customizable): Installs application extensions to any application that supports additional add-on, plugins, etc.
...
* `bin/install_shell` (optional, customizable): Allows you to install and customize your preferred shell. Especially useful if you don't want the macOS default Z shell.
